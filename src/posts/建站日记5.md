...
那么创建时间呢？linux默认不会保存文件的创建时间，但是ext4文件系统会，需要用到[debugfs](https://blog.csdn.net/vic_qxz/article/details/120824368)来获取创建时间。但我们不需要手动去执行这些命令，node已经提前帮我们拿到这个信息了，就是上文中的`birthtime`。
